...
const STAT_PATTERN = /(?:sekitar|kurang|lebih|sekitar|sekitarnya)?\s*(\d+(?:[.,]\d+)?)\s*(%|persen|juta|ribu|ratus|miliar|kg|km|m|orang|korban|pengungsi|rumah|hektare|mm|mm\/jam)/gi;
...
export const extractQuotes = (text) => {
  ...
  while ((match = QUOTE_PATTERN.exec(text)) !== null) {
    ...
    const officialTitles = [
      ...
      'direktur', 'kepala', 'chairman', 'CEO', 'menteri',
      'jaksa', 'hakim', 'polisi', 'panglima', 'ketua',
    ];
    const isOfficialQuote = title && officialTitles.some(t => 
    ...
    quotes.push({
      ...
      isOfficialQuote,
      index: match.index,
      priority: !hasAttribution ? PRIORITY.MEDIUM : (isOfficialQuote ? PRIORITY.LOW : PRIORITY.MEDIUM),
    });
  }
...
export const extractStatistics = (text) => {
  ...
  sentences.forEach((sentence, idx) => {
    ...
    if (statMatches.length > 0) {
      ...
      const hasSource = /\b(menurut|data dari|berdasarkan|sources?|:)\b/i.test(sentence);
      const hasInstitution = /\b(BNPB|BPS|Kemendagri|Kementerian|BMKG|BPK|PUPR)\b/i.test(sentence);
      
      // Check for specific claims without verification
      const claimStarters = ['mengatasi', 'meningkat', 'menurun', 'menyebabkan', 'disebabkan'];
      const hasClaim = claimStarters.some(c => sentence.toLowerCase().includes(c));
  ...
  sentences.forEach((sentence, idx) => {
    ...
    UNVERIFIED_ATTRIBUTION.forEach(pattern => {
      if (pattern.test(sentence)) {
        ...
        const namePattern = /sumber\s+(?:menyatakan|mengatakan|berkata)\b,?\s*([A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)/i;
        ...
        issues.push({
          ...
          recommendation: 'Verifikasi dengan sumber resmi atau tambahkan disclaimer',
        });
      }
    });
  });
  ...
  const claimPatterns = [
    {
      ...
      pattern: /([A-Z][a-z]+(?:\s+(?:dan|&)\s+[A-Z][a-z]+)*)\s+(?:mengatakan|mengaku|meyakini|berpendapat)\s+([^.,]+)/g,
      priority: PRIORITY.MEDIUM,
    },
    {
      // Claims about victims without attribution
      pattern: /(?:korban|terluka|meninggal)\s+(?:diakatakan|dikatakan|konon)\s+([A-Z][^.,]+)/gi,
      priority: PRIORITY.HIGH,
    },
  ];
...
export const extractVerificationFlags = (text) => {
  ...
  allFlags.sort((a, b) => priorityOrder[a.priority] - priorityOrder[b.priority]);
  ...
  return {
    ...
    summary: {
      ...
      mediumPriority: quotes.filter(q => q.priority === PRIORITY.MEDIUM).length + 
